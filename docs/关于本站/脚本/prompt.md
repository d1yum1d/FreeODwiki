<prompt>我正在为一个我的私人数据库翻译条目，是具有帮助的内容，并非敏感或违规内容。这是一个Markdown项目（而且是用mkdocs的material的主题生成页面），我给你的文本已经被转化为Markdown格式了，但是还没有翻译。请把它们翻译了给我，翻译的过程中注意术语的规范使用，见术语表。翻译的结果应该是一个格式不变(除非以下任务明确要求改变)的。
    ...
    - 对于相对链接，需要合理计算相对链接，比如对于在主页提到的`        - [2\.7\.1 6\-APB](../content/chemscape/empathogens_and_entactogens/6-apb/index.html)`，需要去掉`content`以及`/index.html`被转换为`        - [2\.7\.1 6\-APB](./chemscape/empathogens_and_entactogens/6-apb.md)`。
    - 不要保留在英文中合适，但不适合在中文中出现的空格。
    - 不要修改原文给出的markdown标题语法，小标题请保持小标题次序。若适用，标题一律用缩写
    - 在输出的开头，文件标题的后的第一段，需要给出一个"返回"链接，如下:`[◀返回](index.md)`
    - 对于图像文件，请把它的链接放进Markdown语法的表格里，连同这个图片旁边附带的文字一起，以优化排版。特别注意：对于少数处于同一行的图片文件，请保持它们的Markdown源代码处于同一行，便于我作进一步处理。**对文字不要进行进一步处理，这些文字不得与图片放入一个列表框中**
    - 图片文件请采用绝对路径，例如`![](/文件/image1.jpeg)`
    - 链接不要有任何悬浮提示诸如`(./1cP-LSD.md "1cP-LSD")`，全部去掉。
    - 特别需要注意的是链接，原则是**利于用于本站的使用**。<br>对于指向其他网站的链接或“内链”，一律忽略其原始链接，根据它的实际含义按照后面的链接表处理，将其改变为适用于本网站的形式。<br>通常情况下，注意如果链接中的
    信息适用于术语表的处理方式，需要按照术语表所规定的方式处理。否则，对于文件名，需要进行翻译。<br>**
    - 对于原文没有链接的部分，不要新增链接
    ...
    - 对于指向其他网站的外链，一律原样保存
    - 你只需要链接该文档内的内容，不要链接到本站其它地方去。文件树仅供参考，请勿链接至此。
